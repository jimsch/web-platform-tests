var Knowns = [
    {
        name: "Test Case 1 - Basic test case with SHA-256",
        hash:"SHA-256",
    ...
    { 
        ...
        OKM: "085a01ea1b10f36933068b56efa5ad81a4f14b822f5b091568a9cdd4f155fda2c22e422478d305f3f896", // (42 octets)
    },
    {
        ...
        OKM: "0bd770a74d1160f7c9f12cd5912a06ebff6adcae899d92191fe4305673ba2ffe8fa3f1a4e5ad79f3f334b3b202b2173c486ea37ce3d397ed034c7f9dfeb15c5e927336d0441f4c4300e2cff0d0900b52d3b4" // (82 octets)
    },
    {
        ...
        OKM: "0ac1af7002b3d761d1e55298da9d0506b9ae52057220a306e07b6b87e8df21d0ea00033de03984d34918" // (42 octets)
    },
    {
        ...
        OKM: "2c91117204d745f3500d636a62f64f0ab3bae548aa53d423b0d1f27ebba6f5e5673a081d70cce7acfc48" // (42 octets)
    }
];
...
function toUint8Array(input)
{
    ...
    for (i=0; i<input.length; i+= 2) {
        output[i/2] = parseInt(input[i], 16)*16 + parseInt(input[i+1], 16);
    }
    return output;
}

function derive_test(t, data) {
    var key = toUint8Array(data["IKM"]);

    self.crypto.subtle.importKey("raw", key, {name:"HKDF"}, false, ["deriveBits"]).then(
        t.step_func(function(newKey) {
            var params={};

            params["name"] = "HKDF";
            params["hash"] = data["hash"];
            if (data["salt"] != null) params["salt"] = toUint8Array(data["salt"]);
            if (data["info"] != null) params["info"] = toUint8Array(data["info"]);
            
            self.crypto.subtle.deriveBits(params, newKey, data["L"]*8).then(
                t.step_func(function(result) {
                    var known = toUint8Array(data["OKM"]);
                    var result2 = new Uint8Array(result);
                    assert_array_equals(result2, known);
                    t.done();
                }),
                t.step_func(function(err) {
                    assert_unreached("deriveBits should have succeeded err="  + err);
                    t.done();
                })
            );
        }),
...
{
    var iTest;
    for (iTest=0; iTest< Knowns.length; iTest++) {
        t = async_test(Knowns[iTest]["name"]);
        derive_test(t, Knowns[iTest]);
    }
}
