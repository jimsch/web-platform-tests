...
function build_asn_private(keyMap, asnAlg, allData)
{
    // SEQUENCE {
    //    version:0,
    //    algorithm: SEQUENCE {
    ...
    var data = [asn1_encode_integer([new Uint8Array([1])]),
                asn1_encode_OctetString([keyMap["d"]])];

    if (allData) {
        data.push(asn1_encode_tag(0xA0 + 0, [asnAlg["params"]]));
        data.push(asn1_encode_tag(0xA0 + 1, [asn1_encode_bitstring( [build_point(keyMap)])]));
    }

    var x = asn1_encode(0x30, data);
...
var Keys = [
    {key: p256Key, alg:{name:"ECDSA", namedCurve:"P-256"}, oid:{oid: ecPublicKey, params:curveP256} },
    {key: p384Key, alg:{name:"ECDSA", namedCurve:"P-384"}, oid:{oid: ecPublicKey, params:curveP384} },
    {key: p521Key, alg:{name:"ECDSA", namedCurve:"P-521"}, oid:{oid: ecPublicKey, params:curveP521} }
];
...
//
//  Coverage Test cases:
//    1. Import minimal JWK public {kty: ext: key_ops:}
//    2. Import complete JWK public {use: alg: }
//    3. Import minimal JWK private {kty: ext: key_ops:}
//    4. Import complete JWK public {use: alg: }
//    5. Import spki
//    6. Import minimal pkcs8
//    7. Import complete pkcs8
...
function run_test()
{
    ...
    for (j=0; j<Keys.length; j++) {
        ...
        t = async_test("ECDSA Import: jwk public " + key["alg"]["namedCurve"]);
        import_test(t, "jwk", build_jwk_public(key["key"]), key["alg"], true, ["verify"]);

        t = async_test("ECDSA Import: jwk public full " + key["alg"]["namedCurve"]);
        jwk = build_jwk_public(key["key"]);
        jwk["use"] = "sig";
        jwk["alg"] = JwkAlg[key["alg"]["namedCurve"]];
        import_test(t, "jwk", jwk, key["alg"], true, ["verify"]);

        t = async_test("ECDSA Import: jwk private " + key["alg"]["namedCurve"]);
        jwk = build_jwk_private(key["key"]);
        import_test(t, "jwk", jwk, key["alg"], true, ["sign"]);

        t = async_test("ECDSA Import: jwk private full " + key["alg"]["namedCurve"]);
        jwk = build_jwk_private(key["key"]);
        jwk["use"] = "sig";
        jwk["alg"] = JwkAlg[key["alg"]["namedCurve"]];
        import_test(t, "jwk", jwk, key["alg"], true, ["sign"]);

        t = async_test("ECDSA Import: spki " + key["alg"]["namedCurve"]);
        import_test(t, "spki", build_asn_public(key["key"], key["oid"]), key["alg"], true, ["verify"]);

        t = async_test("ECDSA Import: pkcs8 " + key["alg"]["namedCurve"]);
        import_test(t, "pkcs8", build_asn_private(key["key"], key["oid"], false), key["alg"], true, ["sign"]);

        t = async_test("ECDSA Import: pkcs8 full " + key["alg"]["namedCurve"]);
        import_test(t, "pkcs8", build_asn_private(key["key"], key["oid"], true), key["alg"], true, ["sign"]);

        // t = async_test("ECDSA Import: raw " + key["alg"]["namedCurve"]);
        // import_test(t, "raw", build_point(key["key"]), key["alg"], true, ["sign"]);
    }
}
