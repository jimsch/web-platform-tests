...
var asnAbsent = new Uint8Array(0);
var asnSequence = new Uint8Array([0x30]);
...
function asn1_encode_length(dataLength)
{
    ...
    return new Uint8Array([0x83, dataLength/65536, (dataLength/256)%256, dataLength%256]);
}
...
function asn1_encode(tag, dataArray)
{
    ...
    return asn1_to_uint8(result);
}

function asn1_encode_OctetString(dataArray)
{
    var data = asn1_to_uint8(dataArray);
    var length = asn1_encode_length(data.length);

    var result = [new Uint8Array([0x04]), length, data];
...
function asn1_encode_Algorithm(input)
{
    var data = [ input["oid"], input["params"] ];
    data = asn1_to_uint8(data);

    data = [asnSequence, asn1_encode_length(data.length), data];

    return asn1_to_uint8(data);
}
    
function asn1_encode_tag(tag, input)
{
    var data = asn1_to_uint8(input);
    var length = asn1_encode_length(data.length);

    data = [new Uint8Array([tag]), length, data];
